...
with st.expander('More options...', expanded=False):
    data['constrain'] = st.checkbox('Use only content from sources', value=False)
    all = st.checkbox('Search entire dataset (not limited to key papers & posts)', value=False)
    data['namespace'] = 'all-chunks' if all else 'extracted-chunks'
    data['top_k'] = st.slider('Num entries to return', min_value=3, max_value=10, value=5)
    # data['max_history'] = st.slider('Max chat history', min_value=0, max_value=5, value=0)

user_input = st.text_input("Enter your question here", value="What is AI alignment, and why is it important?")
